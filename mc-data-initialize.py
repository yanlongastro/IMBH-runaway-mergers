...
for fs in files:
    ...
    for clst in list(f.keys()):
        ...
        if 'n_particle' not in list(hdf_cls.keys()):
            hdf_cls.create_dataset('n_particle', data=info[cloud][clst]['n_particle'][()])
        ...
        if 'm_c_te3_pmc' not in list(hdf_cls.keys()):
            hdf_cls.create_dataset('t_e_pmc', data = 0.0)
            hdf_cls.create_dataset('m_c_te_pmc', data = 0.0)
            hdf_cls.create_dataset('m_c_te3_pmc', data = 0.0)
        if 'PMC'not in list(hdf_cls.keys()):
            hdf_cls.create_dataset('PMC', data = False)
    print(cloud)
    hdf.flush()
    f.close()
# hdf.close()
